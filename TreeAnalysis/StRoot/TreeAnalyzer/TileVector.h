...
class TileVector
{
    private:
        ...
        Int_t OuterEPDHighRing;

    public:
        TileVector();
        TileVector(StEpdGeom* epdGeom, Int_t EPDID, Double_t vx, Double_t vy, Double_t vz, Int_t InnerEPDHighRing, Int_t OuterEPDLowRing, Int_t OuterEPDHighRing);
...
TileVector::TileVector()
{
    ...
    this->OuterEPDHighRing = 0;
}

TileVector::TileVector(StEpdGeom* epdGeom, Int_t EPDID, Double_t vx, Double_t vy, Double_t vz, Int_t InnerEPDHighRing, Int_t OuterEPDLowRing, Int_t OuterEPDHighRing)
{
    ...
    this->OuterEPDHighRing = OuterEPDHighRing;
}
...
Bool_t TileVector::IsOuterEPD()
{
    ...
    return ((RowNum >= this->OuterEPDLowRing) && (RowNum <= this->OuterEPDHighRing));
}

Bool_t TileVector::IsEPDA()
{
    Int_t RowNum = EPDRow();

    return ((RowNum >= 1) && (RowNum <= 4));
}

Bool_t TileVector::IsEPDB()
{
    Int_t RowNum = EPDRow();

    return ((RowNum >= 5) && (RowNum <= 8));
}

Bool_t TileVector::IsEPDC()
{
    Int_t RowNum = EPDRow();

    return ((RowNum >= 9) && (RowNum <= 12));
}

Bool_t TileVector::IsEPDD()
{
    Int_t RowNum = EPDRow();

    return ((RowNum >= 13) && (RowNum <= 16));
}
