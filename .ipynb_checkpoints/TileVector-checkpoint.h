...
#include "/star/u/mcgordon/VnFromEPD/StRoot/StEpdUtil/StEpdGeom.cxx"
...
class TileVector
{
    ...
    public:
        TileVector();
        TileVector(int EPDID, float vx, float vy, float vz, int InnerEPDHighRing, int OuterEPDLowRing);
...
TileVector::TileVector(int EPDID, float vx, float vy, float vz, int InnerEPDHighRing, int OuterEPDLowRing)
{
    this->EPDID = EPDID;
    this->vx = vx;
    this->vy = vy;
...
int TileVector::EPDSector()
{
    return TMath::Abs(this->EPDID/100);
}

int TileVector::EPDRow()
{
