...
//     std::cout << NSigmaPiBound << std::endl;
    ...
    TH1D *HistoD54;
std::cout << RunIteration << std::endl;
    ...
    if(RunIteration == 4)
    {
        std::string QVectorHistogramFirstString = "/star/data01/pwg/mcgordon/VnFromEPD/V" + std::to_string(nV) + "Histograms/QVectorFirstPassNormal.root";// + argv[3] + ".root";
        char *QVectorHistogramFirstChar = &QVectorHistogramFirstString[0];

        QVectorHistogramFileFirst = TFile::Open(QVectorHistogramFirstChar, "READ");
        
        std::string QVectorHistogramSecondString = "/star/data01/pwg/mcgordon/VnFromEPD/V" + std::to_string(nV) + "Histograms/QVectorSecondPassNormal.root";// + argv[3] + ".root";
        char *QVectorHistogramSecondChar = &QVectorHistogramSecondString[0];
    
        QVectorHistogramFileSecond = TFile::Open(QVectorHistogramSecondChar, "READ");
        
        // std::string ResolutionFileString = "/star/data01/pwg/mcgordon/VnFromEPD/V" + std::to_string(nV) + "Histograms/R_" + std::to_string(nV) + std::to_string(nPsi) + "_Normal.root";// + argv[3] + ".root";
        // const char* ResolutionFileChar = &ResolutionFileString[0];
    
        // ResolutionFile = TFile::Open(ResolutionFileChar, "READ");

        std::string ResolutionFileString = "/star/u/mcgordon/SystematicsCameronResolution/resolutionInfo_INPUT_3p0GeV_averagedRes.root";
        const char* ResolutionFileChar = &ResolutionFileString[0];
...
//     std::cout << AutreeData->Print() << std::endl;   
    ...
    TH1D *HistoInnerEPDQxRaw = 0;
    TH1D *HistoInnerEPDQyRaw = 0;
    TH1D *HistoOuterEPDQxRaw = 0;
    TH1D *HistoOuterEPDQyRaw = 0;
    TH1D *HistoOuterTPCQxRaw = 0;
    TH1D *HistoOuterTPCQyRaw = 0;
    ...
    if(RunIteration == 1)
    {
        ...
        HistoOuterEPDQyRaw = new TH1D("OuterEPDQyRaw", "Qy Raw for Outer EPD; Qy; Events", 250, -QBounds, QBounds);
    ...
    TH2D *EPDPhiVsEta = new TH2D("EPDPhiVsEta","EPD Phi Vs Eta; Eta; Phi", 200, -6, -2.5, 200, -4, 4);
    ...
    TH2F *HistoD4 = new TH2F("DataMSquaredVsQP", "m^2 vs q|p| (Data); q|p| (GeV); m^2 (GeV^2)", 200, -4, 4, 200, -0.1, 10);
    HistoD4->Sumw2();

    TH2F *HistoD4A = new TH2F("DataMSquaredVsQPZoomed", "m^2 vs q|p| (Data); q|p| (GeV); m^2 (GeV^2)", 200, -4, 4, 200, -0.1, 0.15);
    HistoD4A->Sumw2();
    ...
    TH2F *HistoD12 = new TH2F("DatadEdxVsP", "dE/dx vs p (Data); p (GeV); dE/dx (Gev/fm)", 200, 0, 2, 200, 0, 10);
    HistoD12->Sumw2();

    TH2F *HistoD13 = new TH2F("DatadEdxVsPPion", "dE/dx vs p for Pions (Data); p (GeV); dE/dx (Gev/fm)", 200, 0, 2, 200, 0, 10);
    HistoD13->Sumw2();

    TH2F *HistoD14 = new TH2F("DatadEdxVsPKaon", "dE/dx vs p for Kaons (Data); p (GeV); dE/dx (Gev/fm)", 200, 0, 2, 200, 0, 10);
    HistoD14->Sumw2();

    TH2F *HistoD15 = new TH2F("DatadEdxVsPProton", "dE/dx vs p for Protons (Data); p (GeV); dE/dx (Gev/fm)", 200, 0, 2, 200, 0, 10);
    HistoD15->Sumw2();

    TH2F *HistoD16 = new TH2F("DatadEdxVsPDeuteron", "dE/dx vs p for Deuterons (Data); p (GeV); dE/dx (Gev/fm)", 200, 0, 2, 200, 0, 10);
    HistoD16->Sumw2();

    TH2F *HistoD17 = new TH2F("DatadEdxVsPTriton", "dE/dx vs p for Tritons (Data); p (GeV); dE/dx (Gev/fm)", 200, 0, 2, 200, 0, 10);
    HistoD17->Sumw2();

    TH2F *HistoD18 = new TH2F("DataMSquaredVsQPProton", "m^2 vs q|p| for Protons (Data); q|p| (GeV); m^2 (GeV^2)", 200, -4, 4, 200, -0.1, 1.5);
    HistoD18->Sumw2();

    TH2F *HistoD19 = new TH2F("DataMSquaredVsQPKaon", "m^2 vs q|p| for Kaons (Data); q|p| (GeV); m^2 (GeV^2)", 200, -4, 4, 200, -0.1, 1.5);
    HistoD19->Sumw2();

    TH2F *HistoD20 = new TH2F("DataMSquaredVsQPPion", "m^2 vs q|p| for Pions (Data); q|p| (GeV); m^2 (GeV^2)", 200, -4, 4, 200, -0.1, 1.5);
    HistoD20->Sumw2();
    ...
    TH2F *HistoD24 = new TH2F("DataNSigmaPrVsQPProton", "NSigmaPr vs q|p| for Protons (Data); q|p| (GeV); NSigmaPr", 200, -4, 4, 200, -3, 3);
    HistoD24->Sumw2();

    TH2F *HistoD25 = new TH2F("DataNSigmaKaVsQPKaon", "NSigmaKa vs q|p| for Kaons (Data); q|p| (GeV); NSigmaKa", 200, -4, 4, 200, -3, 3);
    HistoD25->Sumw2();

    TH2F *HistoD26 = new TH2F("DataNSigmaPiVsQPPion", "NSigmaPi vs q|p| for Pions (Data); Pt (GeV); NSigmaPi", 200, -4, 4, 200, -3, 3);
    HistoD26->Sumw2();

    TH2F *HistoD27 = new TH2F("DataPtVsYProton", "Pt vs Y for Protons (Data); y_{CM}; Pt (Gev)", 200, -1, 1, 200, 0, 3);
    HistoD27->Sumw2();
    
    TH2F *HistoD27A = new TH2F("DataPtVsYDeuteron", "Pt vs Y for Deuterons (Data); y_{CM}; Pt (Gev)", 200, -1, 1, 200, 0, 5);
    HistoD27A->Sumw2();
    
    TH2F *HistoD27B = new TH2F("DataPtVsYTriton", "Pt vs Y for Tritons (Data); y_{CM}; Pt (Gev)", 200, -1, 1, 200, 0, 7);
    HistoD27B->Sumw2();

    TH2F *HistoD28 = new TH2F("DataPtVsYKaonPlus", "Pt vs Y for K+ (Data); y_{CM}; Pt (Gev)", 200, -1, 1, 200, 0, 2.5);
    HistoD28->Sumw2();

    TH2F *HistoD29 = new TH2F("DataPtVsYKaonMinus", "Pt vs Y for K- (Data); y_{CM}; Pt (Gev)", 200, -1, 1, 200, 0, 2.5);
    HistoD29->Sumw2();

    TH2F *HistoD30 = new TH2F("DataPtVsYPionPlus", "Pt vs Y for Pi+ (Data); y_{CM}; Pt (Gev)", 200, -1, 1, 200, 0, 2.5);
    HistoD30->Sumw2();

    TH2F *HistoD31 = new TH2F("DataPtVsYPionMinus", "Pt vs Y for Pi- (Data); y_{CM}; Pt (Gev)", 200, -1, 1, 200, 0, 2.5);
    HistoD31->Sumw2();
    ...
    TH2F *HistoD32B = new TH2F("DataEPDEastRingVsEta", "EPD East Ring vs Eta; Eta; Ring Number", 200, -6, -1, 200, 0.5, 16.5);
    HistoD32B->Sumw2();
