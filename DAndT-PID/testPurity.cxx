...
void printHistos(TH2D *h2, TString species)
{
  ...
  TF1 *fullFunction_de =
    new TF1("fullFunction_de", "[0]*exp(-0.5*((x-[1])/[2])**2) + [3]*exp(-0.5*((x-[4])/[5])**2) + [6]*exp(-0.5*((x-[7])/[8])**2)", -1.0, 1.0);//-0.7, 0.7);

  TF1 *fullFunction_tr =
    new TF1("fullFunction_tr", "[0]*exp(-0.5*((x-[1])/[2])**2) + [3]*exp(-0.5*((x-[4])/[5])**2) + [6]*exp(-0.5*((x-[7])/[8])**2)", -1.0, 1.0);//-0.7, 0.7);
  ...
  */
  // Low bkg for tritons
  TF1 *bkg0 = new TF1("bkg0", "[0]*exp(-0.5*((x-[1])/[2])**2) + [3]*exp(-0.5*((x-[4])/[5])**2)", -1.0, 1.0);//-0.7, 0.7);
  bkg0->SetLineColor(4);
  bkg0->SetLineStyle(2);

  // Low single bkg
  TF1 *bkg1 = new TF1("bkg1", "[0]*exp(-0.5*((x-[1])/[2])**2)", -1.0, 1.0);//-0.7, 0.7);
  bkg1->SetLineColor(4);
  bkg1->SetLineStyle(2);

  TF1 *signal = new TF1("signal", "[0]*exp(-0.5*((x-[1])/[2])**2)", -1.0, 1.0);//-0.7, 0.7);
  signal->SetLineColor(8);
  signal->SetLineStyle(2);

  // High single bkg
  TF1 *bkg2 = new TF1("bkg2", "[0]*exp(-0.5*((x-[1])/[2])**2)", -1.0, 1.0);//-0.7, 0.7);
  bkg2->SetLineColor(6);
  bkg2->SetLineStyle(2);

  // High double bkg
  TF1 *bkg3 = new TF1("bkg3", "[0]*exp(-0.5*((x-[1])/[2])**2) + [3]*exp(-0.5*((x-[4])/[5])**2)", -1.0, 1.0);//-0.7, 0.7);
  bkg3->SetLineColor(6);
  bkg3->SetLineStyle(2);
  ...
  for (int pBin = /*16*/pBinStart; pBin <= nBins; pBin++)
    {
      ...
      if (species == "de" && pBin == 5)
	{
	  fullFunction_de->SetParameter(0, 2e+9); // Amps
	  fullFunction_de->SetParameter(3, 2.3e+07);
	  fullFunction_de->SetParameter(6, 4.88e+06);
	  ...
	  fullFunction_de->SetParameter(1, -2.1);    // Means
	  fullFunction_de->SetParameter(4, 0.0);
	  fullFunction_de->SetParLimits(4, -0.1, 0.1);
	  fullFunction_de->SetParameter(7, 0.628);
	}
      else if (species == "de" && pBin == 6)
	{
      delete fullFunction_de;
      TF1 *fullFunction_de = new TF1("fullFunction_de", "[0]*exp(-0.5*((x-[1])/[2])**2) + [3]*exp(-0.5*((x-[4])/[5])**2) + [6]*exp(-0.5*((x-[7])/[8])**2)", -1.0, 1.0);        

	  fullFunction_de->SetParameter(0, 5.0e+8); // Amps
	  fullFunction_de->SetParameter(3, 2.3e+07);
	  fullFunction_de->SetParameter(6, 4.88e+06);
	  // fullFunction_de->SetParameter(9, 2.0e+06);

	  fullFunction_de->SetParameter(2, 0.3);   // Std devs
	  //fullFunction_de->SetParLimits(2, 0.1/(2*TMath::Sqrt(2*TMath::Log(2))), 0.55/(2*TMath::Sqrt(2*TMath::Log(2))));
	  fullFunction_de->SetParameter(5, 0.1);
	  //fullFunction_de->SetParLimits(5, 0.1/(2*TMath::Sqrt(2*TMath::Log(2))), 0.55/(2*TMath::Sqrt(2*TMath::Log(2))));
	  fullFunction_de->SetParameter(8, 0.05);
      fullFunction_de->SetParLimits(8, 0.01, 0.20);
	  //fullFunction_de->SetParLimits(8, 0.1/(2*TMath::Sqrt(2*TMath::Log(2))), 0.55/(2*TMath::Sqrt(2*TMath::Log(2))));
	  // fullFunction_de->SetParameter(11, 0.05);
	  
	  fullFunction_de->SetParameter(1, -1.5);    // Means
	  fullFunction_de->SetParameter(4, 0.0);
	  fullFunction_de->SetParameter(7, 0.7);
      fullFunction_de->SetParLimits(7, 0.58, 0.70);
	  // fullFunction_de->SetParameter(10, 0.9);
	}
      else if (species == "de" && pBin == 8)
	{
      delete fullFunction_de;
      TF1 *fullFunction_de = new TF1("fullFunction_de", "[0]*exp(-0.5*((x-[1])/[2])**2) + [3]*exp(-0.5*((x-[4])/[5])**2) + [6]*exp(-0.5*((x-[7])/[8])**2)", -1.0, 1.0);      
	...
	{
	  fullFunction_tr->SetParameter(0, 2e+7); // Amps
	  fullFunction_tr->SetParameter(3, 4e+6);
	  fullFunction_tr->SetParameter(6, 2.4e+06);
	  ...
	  fullFunction_tr->SetParameter(8, 0.05);
	  
	  fullFunction_tr->SetParameter(1, -0.6);    // Means
	  fullFunction_tr->SetParameter(4, 0.15);
	  fullFunction_tr->SetParameter(7, 0.65);
	}
      else if (species == "tr" && pBin == 16)
	{
      delete fullFunction_tr;
      TF1 *fullFunction_tr = new TF1("fullFunction_tr", "[0]*exp(-0.5*((x-[1])/[2])**2) + [3]*exp(-0.5*((x-[4])/[5])**2) + [6]*exp(-0.5*((x-[7])/[8])**2)", -1.0, 1.0);      
	...
	{
	  delete fullFunction_tr;
	  fullFunction_tr = new TF1("fullFunction_tr", "[0]*exp(-0.5*((x-[1])/[2])**2) + [3]*exp(-0.5*((x-[4])/[5])**2) + [6]*exp(-0.5*((x-[7])/[8])**2) + [9]*exp(-0.5*((x-[10])/[11])**2)", -1.0, 1.0);// -0.7, 0.7);
  ...
  for (int i = 0; i < binsUsed.size(); i++)
    {
      ...
      std::cout << h2->GetXaxis()->GetBinLowEdge(bin) << " <= p < " << h2->GetXaxis()->GetBinLowEdge(bin) + h2->GetXaxis()->GetBinWidth(bin) << ": " << "PURITY = " << purities.at(i) << std::endl;
	  std::cout << "   " << lowBounds.at(i) << " < z < " << highBounds.at(i) << std::endl;
	  std::cout << std::endl;
    }
}
...
void testPurity()
{
  ...
  TString fileName = "/star/data01/pwg/mcgordon/VnFromEPD/V6Histograms/BischelNewDatasetFullExtended.root";
