...
def Resolution61(x_val):
    chi = x_val[0]
    arg = chi*chi/4.0
    norm = ROOT.TMath.Sqrt(ROOT.TMath.Pi())/(2.0*ROOT.TMath.Sqrt(2.0))

    besselFiveHalf = ROOT.TMath.Sqrt(1.0/(2.0*ROOT.TMath.Pi()*arg)) * (((6.0/pow(arg,2)) + 2.0) * ROOT.TMath.SinH(arg) - 6.0 * ROOT.TMath.CosH(arg)/(arg) )
    besselSevenHalf = ROOT.TMath.Sqrt(1.0/(2.0*ROOT.TMath.Pi()*arg)) * (((-30/pow(arg,3)) - (12/arg)) * ROOT.TMath.SinH(arg) + ((30/pow(arg,2)) + 2) * ROOT.TMath.CosH(arg))

    y = norm * chi * ROOT.TMath.Exp(-1.0*arg) * (besselFiveHalf + besselSevenHalf)
...
ResolutionFile6 = ROOT.TFile.Open("/star/data01/pwg/mcgordon/VnFromEPD/V6Histograms/R_61_" + sys.argv[1] + ".root", "RECREATE")
...
HistoD54A = ROOT.TH1F("DataResolutionR11", "R_{11}; Centrality; Resolution", 16, 0, 16)
HistoD54A.Sumw2()

HistoD54B = ROOT.TH1F("DataResolutionR21", "R_{21}; Centrality; Resolution", 16, 0, 16)
HistoD54B.Sumw2()

HistoD54C = ROOT.TH1F("DataResolutionR31", "R_{31}; Centrality; Resolution", 16, 0, 16)
HistoD54C.Sumw2()

HistoD54D = ROOT.TH1F("DataResolutionR41", "R_{41}; Centrality; Resolution", 16, 0, 16)
HistoD54D.Sumw2()

HistoD54E = ROOT.TH1F("DataResolutionR51", "R_{51}; Centrality; Resolution", 16, 0, 16)
HistoD54E.Sumw2()

HistoD54F = ROOT.TH1F("DataResolutionR61", "R_{61}; Centrality; Resolution", 16, 0, 16)
HistoD54F.Sumw2()
...
for index in range(1, HistoD51.GetNbinsX() + 1):
    ...
    R11Slope = (R11Bessel.Eval(chi1 + Epsilon) - R11Bessel.Eval(chi1 - Epsilon)) / (2 * Epsilon)

    R21Bessel = ROOT.TF1("R21Bessel",Resolution21,0,10,0)
    R21 = R21Bessel.Eval(chi1)
    R21Slope = (R21Bessel.Eval(chi1 + Epsilon) - R21Bessel.Eval(chi1 - Epsilon)) / (2 * Epsilon)
    Ratio21 = R21Slope / R11Slope
    R21Err = abs(Ratio21) * R11Error
    ...
    R31Slope = (R31Bessel.Eval(chi1 + Epsilon) - R31Bessel.Eval(chi1 - Epsilon)) / (2 * Epsilon)
    Ratio31 = R31Slope / R11Slope
    R31Err = abs(Ratio31) * R11Error
    ...
    R41Slope = (R41Bessel.Eval(chi1 + Epsilon) - R41Bessel.Eval(chi1 - Epsilon)) / (2 * Epsilon)
    Ratio41 = R41Slope / R11Slope
    R41Err = abs(Ratio41) * R11Error
    ...
    R51Slope = (R51Bessel.Eval(chi1 + Epsilon) - R51Bessel.Eval(chi1 - Epsilon)) / (2 * Epsilon)
    Ratio51 = R51Slope / R11Slope
    R51Err = abs(Ratio51) * R11Error
  
    HistoD54E.SetBinContent(index, R51)
    HistoD54E.SetBinError(index, R51Err)

    R61Bessel = ROOT.TF1("R61Bessel",Resolution61,0,10,0)
    R61 = R61Bessel.Eval(chi1)
    R61Slope = (R61Bessel.Eval(chi1 + Epsilon) - R61Bessel.Eval(chi1 - Epsilon)) / (2 * Epsilon)
    Ratio61 = R61Slope / R11Slope
    R61Err = abs(Ratio61) * R11Error
    
    HistoD54F.SetBinContent(index, R61)
    HistoD54F.SetBinError(index, R61Err)



HistoD54A.SetDirectory(ResolutionFile1)
HistoD54B.SetDirectory(ResolutionFile2)
HistoD54C.SetDirectory(ResolutionFile3)
HistoD54D.SetDirectory(ResolutionFile4)
HistoD54E.SetDirectory(ResolutionFile5)
HistoD54F.SetDirectory(ResolutionFile6)
